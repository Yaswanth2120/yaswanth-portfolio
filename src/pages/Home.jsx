...
export const Home = () => {
  return <main>
    ...
    <section id="projects" className="projects">{projects.map((project, projectIndex) => <article className="project-case" key={project.title}><div className="project-copy"><div className="case-meta"><span>0{projectIndex + 1}</span>{projectIndex === 0 && <span>649 AUTOMATED TESTS</span>}</div><h2>{project.title.split("\n").map((line, lineIndex) => <span key={line}>{line}{lineIndex === 0 && project.title.includes("\n") && <br />}</span>)}</h2><p className="description">{project.description}</p><p className="stack">{project.stack.join(" · ")}</p><a href="https://github.com/Yaswanth2120" target="_blank" rel="noreferrer">VIEW ON GITHUB ↗</a></div><Diagram kind={project.kind} /></article>)}</section>
